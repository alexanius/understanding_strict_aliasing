...
Если Вы меняете значение `*intptr`, то значение, на которое указывает `anint` тоже меняется, потому что `*intptr` перекрывает (aliases) `anint`. Это просто другое имя той же вещи.
